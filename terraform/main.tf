...
resource "aws_s3_bucket" "app_bucket" {
  ...
  # FIX CWE-284: Changed from public-read to private
  acl    = "private"
}
...
resource "aws_iam_policy" "app_policy" {
  ...
  # FIX CWE-285: Replace wildcard IAM policy with least-privilege permissions
  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_security_group" "open_sg" {
  ...
  # FIX: Restrict ingress to specific ports and internal CIDR
  ingress {
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
